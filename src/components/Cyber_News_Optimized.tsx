...
export default function CyberNews() {
  const [articles, setArticles] = useState<CyberNewsArticle[]>(staticArticles);
  ...
  // Prioritize critical articles
  const displayArticles = useMemo(() => {
    const critical = sortedArticles.filter((a) => a.severity === "Critical");
    const rest = sortedArticles.filter((a) => a.severity !== "Critical");
    return [...critical, ...rest].slice(0, 25); // Show top 25 articles
  }, [sortedArticles]);

  const featuredArticle = displayArticles[0];
  const gridArticles = displayArticles.slice(1, 13); // Show 12 in grid
  ...
  // Auto-fetch RSS on mount if online
  useEffect(() => {
    if (isOnline && !useLiveData) {
      handleFetchRSS();
    }
  }, []);
  ...
  const handleFetchRSS = useCallback(async () => {
    if (!isOnline) {
      alert("Cannot fetch RSS feeds while offline");
      return;
    }

    setIsLoading(true);
    try {
      const rssArticles = await fetchAllRSSArticles(15); // 15 articles per source
      
      if (rssArticles.length > 0) {
        setArticles(rssArticles);
        setUseLiveData(true);
        setLastUpdate(new Date());
        console.log(`✓ Loaded ${rssArticles.length} live articles`);
      } else {
        console.warn("No RSS articles fetched, using static data");
        alert("No articles could be fetched from RSS feeds. Using static data.");
      }
    } catch (error) {
      console.error("RSS fetch error:", error);
      alert("Failed to fetch RSS feeds. Using static data.");
    } finally {
      setIsLoading(false);
    }
  }, [isOnline]);
  ...
  const handleUseStaticData = useCallback(() => {
  ...
  const stats = useMemo(() => ({
    ...
    todayCount: displayArticles.length,
    ...
    <div className="relative min-h-screen bg-black">
      ...
      >
        ...
        >
          ...
          >
            {useLiveData ? (
              <>Real-time cybersecurity intelligence from {stats.sources} live RSS feeds — automatically updated every 15 minutes.</>
            ) : (
          ...
          >
            ...
            {useLiveData && (
              <div className="flex items-center gap-2 px-4 py-2 rounded-full
                              ...
                              border border-green-400/30 text-green-300 text-xs font-bold">
            ...
            {useLiveData && (
              <motion.button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-full
                           bg-indigo-500/15 border border-indigo-400/40 text-indigo-400
                           hover:bg-indigo-500/25 hover:border-indigo-400/60
                           transition-all duration-300 text-xs font-semibold"
              >
                Use Static Data
              </motion.button>
        ...
        >
          ...
          <StatCard icon={Eye}           value={stats.todayCount} label="Displayed"       colorClass="text-violet-400" bgClass="bg-violet-500/15" borderClass="border-violet-400/30" />
        </motion.div>
        ...
        >
          ...
          <div className="flex items-center gap-3 px-6 py-3 rounded-2xl
            ...
            <span className="text-cyan-300 font-black text-xs tracking-[0.15em] uppercase">
              {useLiveData ? "Latest Intel" : "Featured Stories"}
            </span>
            ...
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">{RSS_SOURCES.filter(s => s.enabled).length} active feeds</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {RSS_SOURCES.filter(s => s.enabled).slice(0, 10).map((src, i) => (
                  <motion.a
                    key={i}
                    href={src.url}
                    target="_blank"
            ...
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">Real-time aggregation engine</p>
                </div>
              </div>
                ...
                {[
                  {
                    ...
                    title: "Newest First",
                    desc: "Articles are automatically sorted by publish date with newest content appearing first.",
                  },
                  {
                    icon: AlertTriangle,
                    color: "text-red-400",
                    ...
                    desc: "Critical severity articles are always pinned to the top regardless of date.",
                  },
                  {
                    icon: Zap,
                    color: "text-yellow-400",
                    ...
                    desc: "AI-powered categorization and severity detection based on content analysis.",
                  },
                ].map((item, i) => {
              ...
              <div className="mt-4 p-4 rounded-xl bg-indigo-950/50 border border-indigo-400/25">
                <p className="text-indigo-300 text-xs font-semibold leading-relaxed">
                  <span className="font-black">Live RSS Active!</span> Articles are fetched from {stats.sources} sources 
                  and cached for 15 minutes. Click "Refresh Feed" to fetch the latest updates.
                </p>
              </div>
            </div>
          </div>
